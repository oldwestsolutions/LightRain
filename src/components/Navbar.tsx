...
export function Navbar({ user, searchQuery, onSearchChange, onLogout }: Props) {
  ...
  return (
    <>
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-neutral-200/90 bg-white/95 shadow-nav backdrop-blur-md safe-pt">
        ...
        {mobileOpen && (
          <div className="max-h-[min(70vh,calc(100dvh-3.5rem))] overflow-y-auto overscroll-contain border-t border-neutral-100 bg-white px-4 py-4 safe-pb lg:hidden">
            ...
            <button
              ...
              className="mt-4 flex min-h-[48px] w-full touch-manipulation items-center justify-center gap-2 rounded-xl border border-neutral-200 py-3 text-base font-medium text-neutral-800 active:bg-neutral-50"
            >
              <LogOut className="h-4 w-4" />
              Log out
            </button>
